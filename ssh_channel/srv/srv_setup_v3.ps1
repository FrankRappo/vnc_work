...
# 1. OpenSSH Server — GitHub zip ONLY (no Windows Update / Get-WindowsCapability; WU is slow here)
if(Test-Path 'C:\\Windows\\System32\\OpenSSH\\sshd.exe'){
  L 'sshd.exe already present -> skip install'
} else {
  L 'installing OpenSSH from GitHub zip (bypassing WU)'
  try{
    ...
    $env:Path += ';C:\\Program Files\\OpenSSH\\OpenSSH-Win64'
    L 'GitHub OpenSSH installed'
  }catch{ L ('github install ERR: '+$_.Exception.Message) }
}
